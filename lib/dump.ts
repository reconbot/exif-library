...
export const dump = (originalExifObj: IExif): string => {
  ...
  if (
    ...
    (exifObj.Interop && Object.keys(exifObj['Interop']).length)
  ) {
    zerothIfd[TagNumbers.ImageIFD.ExifTag] = 1
    exifIfd = exifObj['Exif'] || {}

    if (exifObj.Interop && Object.keys(exifObj['Interop']).length) {
  ...
  } else if (Object.keys(zerothIfd).indexOf(TagNumbers.ImageIFD.ExifTag.toString()) > -1) {
  ...
  const zerothIfdLength =
    ...
    Number(Boolean(exifIfd)) * 12 +
    Number(Boolean(gpsIfd)) * 12 +
    4 +
